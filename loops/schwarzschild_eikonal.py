...
aims = aim.aim_solver(l0,s0,x=y,x0=y0,n_iter=n,l_mom=l_val)
...
for L in range(1, 101):
    ...
    l0 = (4*m*I*w*(2*y**2 - 4*y + 1) - (1 - 3*y)*(1-y) ) / (y*(1-y)**2) 
    s0 = ( 16*m**2*w**2*(y-2) - 8*m*I*w*(1-y) + l*(l+1) + (1-s**2)*(1-y) ) / ( y*(1-y)**2 )
    # Substitute values (Looping over L momentum)
    l0 = l0.subs(m,m_val).subs(l,L).subs(s,s_val)
    s0 = s0.subs(m,m_val).subs(l,L).subs(s,s_val)
    # Re-calculate new position
    y0 = ( 1 - 2*m / ( 3*m/2 * 1/(l*(l+1)) * ( l*(l+1) - (1-s**2) + sym.sqrt( l**2*(l+1)**2 + 14/9*l*(l+1)*(1-s**2) + (1-s**2)**2 ) ) ) ).subs(m, m_val).subs(l, l_val).subs(s, s_val).evalf(75)

    print("\nL = " + str(L) + "\n")
    # Initialize method with new parameters
    aims.__init__(l0,s0,x=y,x0=y0,n_iter=n, l_mom=L)
