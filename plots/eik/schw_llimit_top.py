...
data = np.loadtxt("schw_llimit.txt", skiprows=0)
...
#ax_re.plot(data[:,0], f(data[:,0], *popt), color="dimgrey", alpha=1, linestyle="--", marker="")

ax_re.legend(loc = "upper left", bbox_to_anchor=(1, 0.27))
#ax_im.legend(loc = "upper left")

ax_re.set_title(r"Límite $eikonal$ de los modos cuasinormales")
ax_im.set_xlabel(r"Momento angular $\ell$")
#ax_re.set_xlabel(r"$y_0$")

plt.savefig("schw_llimit_topbot.pdf", bbox_inches="tight")
plt.show()