...
data = np.loadtxt("pt_pos.txt", skiprows=0)

f, (ax_re, ax_im) = plt.subplots(2, 1, sharex=True, facecolor='w')

plt.subplots_adjust(wspace=0, hspace=0)

ax_re.set_title("Región de convergencia Pöschl-Teller")
...
ax_re.vlines(x=0, ymin=-10.55, ymax=10.75, linestyle="-", color="dimgrey", label=r"$x_{\text{max}}$", linewidth=0.95)
#ax_im.vlines(x=0.3903882032022075, ymin=-1.55, ymax=1.25, linestyle="--", color="dimgrey", label=r"$y_{\text{max}}$", linewidth=0.95)
ax_im.vlines(x=0, ymin=-10.55, ymax=10.75, linestyle="-", color="dimgrey", label=r"$x_{\text{max}}$", linewidth=0.95)

ax_re.set_ylim(-0.1,1.25)
ax_im.set_ylim(-4.75,0.75)
...
ax_re.legend(loc = "upper left", bbox_to_anchor=(1, 0.38))
#ax_im.legend(loc = "upper left")

#ax_re.set_title("Modos cuasinormales P-T")
ax_im.set_xlabel(r"Punto de evaluación $x_0$")
#ax_re.set_xlabel(r"$y_0$")

plt.savefig("pt_pos.pdf", bbox_inches="tight")
plt.show()